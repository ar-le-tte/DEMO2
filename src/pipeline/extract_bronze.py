...
def download_movies_parallel(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        future_map = {ex.submit(fetch_and_save, mid): mid for mid in movie_ids}
    ...
    logger.info("Download finished. statuses=%s", count_statuses(results))
    return results


def count_statuses(results: List[Tuple[int, str]]) -> dict:
    counts: dict[str, int] = {}
